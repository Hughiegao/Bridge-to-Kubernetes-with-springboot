...
public class TodoListController {
    ...
    @Value("#{environment.URI_USERS}")
    private String URI_USERS;
    
    @RequestMapping(value = "/api/todolist/getallusers",
            method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<?> getUserItem() {
        try {
            System.out.println(URI_USERS);
            RestTemplate restTemplate = new RestTemplate();       
            return new ResponseEntity<>(restTemplate.getForObject(URI_USERS, UserItem[].class), HttpStatus.OK);
        } catch (Exception e) {
